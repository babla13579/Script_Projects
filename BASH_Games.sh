...
Set_Initial_Vars(){
	...
	Banner_File="${Script_Temp_Dir}/${Main_User}/Dice_Script_Banner-${StartTime}"
...
Create_Config_File(){
	...
	echo "Dev_Mode = 0" >> $Config_File
	echo "MOTD = \"BASH Linux Script   \"" >> $Config_File
	...
	echo "sleep_max = 10000" >> $Config_File
}
...
Read_Config_File(){
	...
	if [ -z "$MOTD" ];then MOTD=$(cat $Config_File | grep "^MOTD =" | grep -oP '(?<=MOTD = ").*(?=")')
	fi
...
Menu_Timer(){
	unset choice
	if [[ "$(( $New_Menus_Size - $Old_Menus_Size ))" -ge "2" ]] || [[ "$(( $New_Menus_Size - $Old_Menus_Size ))" -le "-2" ]];then
...
Print_User_Config_Menu(){
	...
	echo -e -n "5  Toggle:     Show Warnings?                            Currently = ";if [ "$Show_Warnings" -eq "0" ];then echo -e "${Cust_Error}No${STD}";else echo -e "${Cust_Output}Yes (Recommended)${STD}";fi
	...
	echo -e -n "F  Set:        User to emulate                           Currently = ";if [ -z "$Main_User_Override" ];then echo -e "${Cust_Output}Not Emulating${STD}";else echo -e "${Cust_Error}Yes  ----  $Main_User_Override${STD}";fi
...
Read_Menu_User_Config_Menu(){
	case $choice in
		...
		f|F) ls -b1 $Script_Temp_Dir;read -p "Set User to Emulate...  (Blank to reset):   " Main_User_Override;Set_Initial_Vars ;;
...
Print_Dice_Menu(){
	if [[ "$Menu_Banner" != "Lets Roll Some Dice!   " ]];then Menu_Banner="Lets Roll Some Dice!   ";Read_Dice_Config;fi
...
Read_Menu_Dice_Menu(){
	case $choice in
		...
		j|J) if [[ "$Auto_Roll" -eq "1" ]];then if [[ "$Points" -ge "$Max_Idle_Spins_Cost" ]];then Max_Idle_Spins=$(echo "$Max_Idle_Spins * 5" | bc -l);Points=$(echo "$Points - $Max_Idle_Spins_Cost" | bc -l);Max_Idle_Spins_Cost=$(echo "$Max_Idle_Spins_Cost * 30" | bc -l);fi;fi ;;
		/|\?) Roll_Da_Dice ;;
		\@) Roll_Da_Dice ;;
		s|S) G_Dice_Stats ;;
		:) Dice_Var_Dump ;;
